...
emP = {}
Tunnel.bindInterface("motta_drugs",emP)
local idgens = Tools.newIDGenerator()
-----------------------------------------------------------------------------------------------------------------------------------------
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookdrugs = "" --AQUI SERÃO AS LOGS.
...
function emP.Quantidade()
	...
	if quantidade[source] == nil then
	   quantidade[source] = math.random(1,7)	
	end
...
function emP.checkPayment()
	...
	if user_id then
		...
		and vRP.getInventoryItemAmount(user_id, 'lancaperfume') <= 0 then
			TriggerClientEvent("Notify",source,"negado","Número insuficiente de <b>Pacotes</b>.")
		else
			local policia = vRP.getUsersByPermission("permissãodapoliciadatuabase.permissao")
			local valorDroga = math.random(300,410) 
				if #policia < 2 then 
					valorDroga = math.random(300,410) 
				elseif #policia >= 2 then
					valorDroga = math.random(2000,2500) 
				elseif #policia >= 4 then
					valorDroga = math.random(3000,4500) 
				elseif #policia >= 6 then
					valorDroga = math.random(8000,11000)
				elseif #policia >= 8 then
					valorDroga = math.random(15000,20000)
				end 
...
function emP.MarcarOcorrencia()
	...
	if user_id then
		local soldado = vRP.getUsersByPermission("permisãodapoliciadatuabase.permissao")
