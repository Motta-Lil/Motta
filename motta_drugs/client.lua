...
emP = Tunnel.getInterface("motta_drugs")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIÁVEIS (Todos Créditos A Motta.)
-----------------------------------------------------------------------------------------------------------------------------------------
local blips = false
...
-----------------------------------------------------------------------------------------------------------------------------------------
local pontos = {
	{ 89.56,-1745.31,30.08 }, -- Coloque o CDS para pegar rota.
}

local locs = {
    [1] = { ['x'] = -14.09, ['y'] = -1442.08, ['z'] = 31.11 }, --Aqui para baixo serão locais de entregas.
		...
		for _,mark in pairs(pontos) do
			...
			if distance <= 0.2 then
				...
				if not entregando then
					if distance <= 0.2 then
						drawTxt("PRESSIONE  ~y~E~w~  PARA INICIAR AS ROTAS",4,0.5,0.92,0.35,255,255,255,180)
	...
	while true do
		...
		if entregando then
			...
			if distance <= 0.5 then
				...
				if distance <= 0.2 then
					drawTxt("PRESSIONE  ~y~E~w~  PARA ENTREGAR OS PACOTES",4,0.5,0.92,0.35,255,255,255,180)
					if IsControlJustPressed(0,38) then
			...
			if entregando then
				drawTxt("~y~PRESSIONE ~r~F8 ~w~PARA FINALIZAR AS ENTREGAS",4,0.230,0.905,0.35,255,255,255,200)
				drawTxt("VÁ ATÉ O DESTINO E ENTREGUE ~g~"..quantidade.."x~w~ PACOTES",4,0.230,0.93,0.35,255,255,255,200)
			  end
